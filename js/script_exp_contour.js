...
3000, 2.768441`,
  ...
  function destPoint(latDeg, lonDeg, distanceM, bearingDeg){
    ...
    const θ = Math.PI/180 * bearingDeg, δ = distanceM / R_EARTH;
  ...
  function setLabelButton(){
    ...
    b.textContent = (settings.showLabel ? '👁 Label: On' : '🚫 Label: Off');
  }
  ...
  function ensureCenterMarker(lat, lon, name){
    ...
    const label = name ? `📍 ${esc(name)}` : `📍 ${lat.toFixed(5)}, ${lon.toFixed(5)}`;
  ...
  function refreshLegend(){
    ...
    leg.innerHTML = `<b>Kontur Ledakan</b>
      <div>Daerah: ${placeName ? esc(placeName) : '<span class="muted">memuat…</span>'}</div>
  ...
  function updatePlayLabel(){ document.getElementById('btnPlay').textContent = playing ? '⏸ Pause' : '▶ Play'; }
